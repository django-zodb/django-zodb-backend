...
- "Create table"  → ensure a LOBTree container exists in the ZODB root
- "Drop table"    → delete the LOBTree container from the root
...
class DatabaseSchemaEditor(BaseDatabaseSchemaEditor):
    ...
    def create_model(self, model):
        """Ensure the ZODB collection (LOBTree) for this model exists."""
        self.connection.ensure_collection(model._meta.db_table)

    def delete_model(self, model):
        """Drop the ZODB collection for this model."""
    ...
    def alter_db_table(self, model, old_db_table, new_db_table):
        """Rename a collection by copying its BTree and deleting the old one."""
